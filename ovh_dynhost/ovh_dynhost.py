...
def get_conf(conf_path, hostname=None, username=None, password=None):
    global LOGGER

    if not os.path.exists(conf_path):
        LOGGER.warning(
            "No config file found at {conf_path}".format(conf_path=conf_path)
        )
...
def main():
    ...
    (hostname, username, password) = get_conf(
        conf_path=conf_file,
        hostname=hostname,
        username=username,
        password=password,
    ...
    # Building Request
    payload = {"myip": ip, "system": "dyndns", "hostname": hostname}

    headers = {"user-agent": "ovh-dynhost/" + __version__}
